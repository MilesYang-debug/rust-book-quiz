...
所有产物由 [GitHub Actions 自动构建](#自动构建与发布github-actions)，也可以按下面的指南手工编译。
...
> Node.js 不是编译依赖——只有校验题库（`node validate.js`）和重新生成内嵌题库快照时才用到。
...
```bash
sudo apt update
sudo apt install -y libwebkit2gtk-4.1-dev build-essential curl wget file libssl-dev libgtk-3-dev librsvg2-dev libxdo-dev
```
...
$ANDROID_HOME/build-tools/34.0.0/apksigner verify RustBookQuiz.apk   # ✅ 无报错即签名有效
```
...
**桌面端（零编译）**：写 `bank/chNN.json` → `node validate.js` 校验 → 把 `bank/` 放到可执行文件旁 → 重启应用即生效。
