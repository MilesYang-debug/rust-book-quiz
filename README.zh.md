...
一套 Rust 代码，四端交付：Windows / Linux 桌面 · Android · Web 浏览器
无 JS 框架 · 无后端 · 离线可用
...
| [`.github/workflows/`](.github/workflows/) | CI：`release.yml`（四端产物）· `deploy-pages.yml`（在线版）· `validate.yml`（题库门禁） |
| [`app/quiz-bank/`](app/quiz-bank/) | 题库 schema 类型 + 校验器（`cargo run -p quiz-bank`） |
...
| [`app/src-tauri/tauri.conf.json`](app/src-tauri/tauri.conf.json) | 窗口配置 + 打包配置（deb / AppImage） |
