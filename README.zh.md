...
**《The Rust Programming Language》（官方 Rust Book）配套刷题应用**

一套 Rust 代码，五端交付：Windows / macOS / Linux 桌面 · Android · Web 浏览器
无 JS 框架 · 无后端 · 离线可用
...
中文 | [English](README.md) | [🎮 在线体验](https://MilesYang-debug.github.io/rust-book-quiz/)
...
官方学习资源各管一环，本项目补的是它们都没覆盖的**自测**环节：

| 资源 | 学习方式 | 训练的能力 |
|---|---|---|
| [The Book](https://doc.rust-lang.org/book/) | 系统阅读 | 建立概念框架 |
| [Rust by Example](https://doc.rust-lang.org/rust-by-example/) | 读可运行的示例 | 看懂惯用写法 |
| [rustlings](https://github.com/rust-lang/rustlings) | 修代码让它编译通过 | 动手，和借用检查器搏斗 |
| **本项目** | 刷题 + 解析 + 错题本 | **检验理解、暴露误解** |

读书容易"看懂就忘"，rustlings 有编译器当拐杖；而这里的 **Code Output**（不运行，预测输出）和 **Misconception**（针对常见误解）题型，要求你在没有编译器反馈的情况下模拟 Rust 的语义——这是主动回忆式的检测，也是所有权、`Deref` 强制转换、`match` 穷尽性这类反直觉细节真正巩固下来的方式。错题自动收录、答对移出，形成复习闭环。

推荐用法（题目按 Book 章节小节对齐，可无缝插入读书流程）：

> **读一章 Book → 刷本项目对应章节 → 错题回读原文小节 → 用 rustlings / 小项目动手**

刷题只是"检测"，写代码才是"训练"——把它当作读书之后、动手之前的自测关卡即可。
...
| **macOS** | [Releases](../../releases) 下载 `RustBookQuiz.dmg` | Universal 包（Intel + Apple Silicon 通用）。未做公证：首次打开需右键应用 →"打开" |
...
| **自部署 Web 版** | [Releases](../../releases) 下载 `RustBookQuiz-web.zip` | 解压丢到任意静态服务器，见 [5. Web 版](#5-web-版) |
...
- `section`：必须写成与 The Book 目录一致的 `"N.M Title"` —— 校验器会检查，
  应用也靠它自动生成每道题的"查看原文"链接（映射表见 `app/quiz-bank/src/book_toc.rs`）
- `anchor`（可选）：该小节页面 URL 中 `#` 后面的锚点
  （如 `"installing-rustup-on-linux-or-macos"`）——让"查看原文"直接跳到具体标题，
  而不是页面顶部

出题质量准则与完整 PR 清单见 [CONTRIBUTING.zh.md](CONTRIBUTING.zh.md)。
...
cargo run -p quiz-bank --bin code_check            # 用真实 rustc 机械验证代码片段的行为断言
cargo run -p quiz-bank --features net --bin link_check   # 核对 The Book 链接与锚点（CI 每周自动跑）
```
...
```mermaid
flowchart LR
    bank[("📚 bank/chNN.json<br/>题库源数据")]
    snap["assets/bank.json<br/>内嵌快照（生成物）"]
    front["Leptos 0.6 前端<br/>Rust → WASM"]
    shell["Tauri 2 壳"]
    win["Windows<br/>WebView2"]
    linux["Linux<br/>WebKitGTK"]
    android["Android<br/>系统 WebView"]
    web["Web<br/>纯静态·无壳"]

    bank -->|同步快照| snap
    snap -- 编译期内嵌 --> front
    bank -. 桌面端运行时热加载 .-> shell
    front --> shell
    front --> web
    shell --> win
    shell --> linux
...
| 层 | 技术 | 职责 |
|---|---|---|
| **前端** | Leptos 0.6（Rust → WASM） | 全部业务逻辑（判分 / 多选 / 抽题 / 错题本）· 纯 Rust 语法高亮器（无 JS 依赖）· 内嵌 Inter + JetBrains Mono 字体，多端渲染一致 |
| **壳** | Tauri 2 | 提供 `load_bank_files` / `open_url` / 窗口控制命令 · Windows 走 WebView2，Linux 走 WebKitGTK，Android 用系统 WebView · Web 版无壳直接跑 |
| **题库** | JSON（每章一个文件） | 唯一数据源 `bank/chNN.json` · 桌面端运行时读可执行文件旁的 `bank/` · 移动 / Web / 回退用编译期内嵌快照 |

## 目录结构

| 路径 | 说明 |
|---|---|
| [`bank/`](bank/) | 题库源数据 `chNN.json`，每章一个文件（**唯一数据源**） |
| [`.github/workflows/`](.github/workflows/) | CI：`release.yml`（五端产物）· `deploy-pages.yml`（在线版）· `validate.yml`（题库门禁） |
| [`app/quiz-bank/`](app/quiz-bank/) | 题库 schema 类型 + 三件机械工具：校验器（`cargo run -p quiz-bank`）、`code_check`（rustc 实跑验证代码题断言）、`link_check`（核对 The Book 链接锚点，需 `--features net`） |
| [`app/index.html`](app/index.html) | trunk 构建入口（声明字体 / 图标资源拷贝） |
| [`app/style.css`](app/style.css) | 暗色科技风主题 + 浅色主题 + 移动端媒体查询 |
| [`app/fonts/`](app/fonts/) · [`ferris.png`](app/ferris.png) | 内嵌资源（OFL 字体 / 自绘 Ferris） |
| [`app/assets/bank.json`](app/assets/bank.json) | 内嵌题库快照（**生成物，勿手改**） |
| [`app/src/main.rs`](app/src/main.rs) | 应用壳：路由 / 标题栏 / 主题 / 抽屉 |
| [`app/src/model.rs`](app/src/model.rs) | 题库加载、语法高亮（schema 类型复用 quiz-bank） |
| [`app/src/storage.rs`](app/src/storage.rs) | localStorage 持久化（分数 / 错题 / 主题） |
| [`app/src/views.rs`](app/src/views.rs) | Sidebar / Home / Exam / Wrong / Quiz 组件 |
| [`app/src-tauri/src/lib.rs`](app/src-tauri/src/lib.rs) | Tauri 壳入口 + 全部命令（移动端入口在此） |
| [`app/src-tauri/src/main.rs`](app/src-tauri/src/main.rs) | 桌面入口（调用 `lib::run`） |
| [`app/src-tauri/tauri.conf.json`](app/src-tauri/tauri.conf.json) | 窗口配置 + 打包配置（deb / AppImage / dmg） |
| [`app/src-tauri/capabilities/`](app/src-tauri/capabilities/) | 窗口拖拽权限 |
| [`app/src-tauri/icons/`](app/src-tauri/icons/) | `icon.ico`（Windows）/ `icon.png`（移动 + Linux 打包） |
| [`app/src-tauri/gen/android/`](app/src-tauri/gen/android/) | Android 工程（已提交，无需重新 init） |

## 自动构建与发布（GitHub Actions）

日常开发**不需要**在本地凑齐五个平台的环境——CI 全部代劳：

| 工作流 | 触发 | 产出 |
|---|---|---|
| [release.yml](.github/workflows/release.yml) | 推送 `v*` 标签 | exe / deb / AppImage / Linux 裸二进制 / macOS dmg / apk / web zip，自动挂到 Releases |
...
| [link-check.yml](.github/workflows/link-check.yml) | 每周定时 + 手动触发 | 抓取题库引用的全部 The Book 页面，核对页面与每题锚点仍然存在（原书偶尔会重构标题） |
...
**第 1 步**：安装 Rust 工具链（stable，1.85+——各 crate 使用 edition 2024）。访问 <https://rustup.rs/> 按提示安装，然后验证：
...
前置：完成[公共准备](#公共准备所有平台都要做一次)；以下步骤必须**在 macOS 上**执行。无需额外系统依赖——WebView（WKWebView）系统自带。

**第 1 步**：安装 Tauri CLI（与上面 Linux 的两种方式相同）。

**第 2 步**（仅打 universal 包需要）：补齐两个 Apple 目标：
...
```bash
cd app
cargo tauri build --bundles dmg                                     # 仅当前架构
cargo tauri build --target universal-apple-darwin --bundles dmg     # Intel + Apple Silicon（CI 发布用的就是这个）
# ✅ 产物：src-tauri/target/release/bundle/dmg/*.dmg
#    （universal 包在 src-tauri/target/universal-apple-darwin/release/bundle/dmg/*.dmg）
```

> ⚠️ dmg 未签名、未公证，首次启动会被 Gatekeeper 拦下——右键应用 →"打开"即可
> （或清除隔离标记：`xattr -cr /Applications/RustBookQuiz.app`）。
...
cargo install tauri-cli --locked    # 或 cargo binstall tauri-cli（秒装预编译版）
```

> ⚠️ Android 构建**必须用 cargo 版 tauri-cli**：`gen/android` 的 gradle 任务（buildSrc/BuildTask.kt）
> 会回调 `cargo tauri`，只装 npm 版 `@tauri-apps/cli` 会报 ``no such command: `tauri` ``。
...
| Android 构建 ``no such command: `tauri` `` | gradle 会回调 `cargo tauri`，npm 版 CLI 不够：`cargo install tauri-cli --locked`（或 `cargo binstall tauri-cli`） |
